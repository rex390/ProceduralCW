...
bool TerrainShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC ModeBufferDesc;
	...
	//setup the buffer for the Mode
	ModeBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	ModeBufferDesc.ByteWidth = sizeof(LightBufferType);
	ModeBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	ModeBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	ModeBufferDesc.MiscFlags = 0;
	ModeBufferDesc.StructureByteStride = 0;

	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result = device->CreateBuffer(&ModeBufferDesc, NULL, &m_ModeBuffer);
	if(FAILED(result))
	{
		return false;
	}
											 ...
											 D3DXMATRIX projectionMatrix, D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor, D3DXVECTOR3 lightDirection)
{
	...
	// Lock the tessellation constant buffer so it can be written to (ModeBuffer).
	result = deviceContext->Map(m_ModeBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	dataPtr3 = (ModeBufferType*)mappedResource.pData;

	
	dataPtr3->Mode = GetMode();
	dataPtr3->PositionX =GetDiggerX();
	dataPtr3->PositionZ = GetDiggerZ();
	dataPtr3->ExtraWidthHeight = GetDiggerWidthHeight();

	
	// Unlock the tessellation constant buffer.
	deviceContext->Unmap(m_ModeBuffer, 0);
	bufferNumber = 1;

	//	// Now set the tessellation constant buffer in the hull shader with the updated values.
	//deviceContext->DSSetConstantBuffers(bufferNumber, 1, &m_ScaleBuffer);
	deviceContext->VSSetConstantBuffers(bufferNumber,1, &m_ModeBuffer);
...
void TerrainShaderClass::IncreaseMode()
{
	DrawMode++;
}

void TerrainShaderClass::DecreaseMode()
{
	DrawMode--;
}

int TerrainShaderClass::GetMode()
{
	return DrawMode;
}

int TerrainShaderClass::GetDiggerX()
{
	return DiggerPositionX;
}

int TerrainShaderClass::GetDiggerZ()
{
	return DiggerPositionZ;
}


void TerrainShaderClass::IncreaseDiggerZ()
{
	DiggerPositionZ++;
}

void TerrainShaderClass::DecreaseDiggerZ()
{
	DiggerPositionZ--;
}

void TerrainShaderClass::IncreaseDiggerX()
{
	DiggerPositionX++;
}

void TerrainShaderClass::DecreaseDiggerX()
{
	DiggerPositionX--;
}

void TerrainShaderClass::IncreaseDiggerWidthHeight()
{
	DiggerWidthHeight++;
}

void TerrainShaderClass::DecreaseDiggerWidthHeight()
{
	DiggerWidthHeight--;
}
int TerrainShaderClass::GetDiggerWidthHeight()
{
